...
void drawArrwow(SDL_Renderer* renderer, int x1, int y1, int x2, int y2, int x3, int y3) {
    ...
    if (y1 > y3) { int temp; temp = y1; y1 = y3; y3 = temp; temp = x1; x1 = x3; x3 = temp; }
    if (y2 > y3) { int temp; temp = y2; y2 = y3; y3 = temp; temp = x2; x2 = x3; x3 = temp; }
...
void* chequeQueue(void* arg) {
    ...
    while (1) {
        for (int i = 0; i < 4; i++) sharedData->nextLight[i] = 0;
        sleep(5);
        for (int i = 0; i < 4; i++) sharedData->nextLight[i] = 1;
        sleep(5);
    }
}
